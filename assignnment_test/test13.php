...
// Add GBP manually with a rate of 1
$gbp_data = getLocationsAndCurrencyName('GBP', $currency_countries);
$gbp_currency_element = $rates_xml->addChild('currency');
$gbp_currency_element->addAttribute('rate', '1');
$gbp_currency_element->addAttribute('live', updatelive('GBP'));
$gbp_currency_element->addChild('code', 'GBP');
$gbp_currency_element->addChild('curr', htmlspecialchars($gbp_data['curr']));
if (!empty($gbp_data['loc'])) {
    $gbp_currency_element->addChild('loc', implode(', ', $gbp_data['loc']));
}
...
foreach ($currency_data->children() as $currency_code => $rate) {
    ...
    if (!empty($data['loc'])) {
        $currency_element->addChild('loc', implode(', ', $data['loc']));
    }
}
}
